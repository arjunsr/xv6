...
bootmain(void)
{
  ...
  if(elf->magic != ELF_MAGIC)
    ...
    7d49:	45 4c 46 
    7d4c:	75 50                	jne    7d9e <bootmain+0x7f>
    goto bad;
  ...
  eph = ph + elf->phnum;
    ...
    7d67:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; ph < eph; ph++)
    7d6a:	eb 21                	jmp    7d8d <bootmain+0x6e>
    readseg(ph->va & 0xFFFFFF, ph->memsz, ph->offset);
    7d6c:	8b 43 04             	mov    0x4(%ebx),%eax
    7d6f:	89 44 24 08          	mov    %eax,0x8(%esp)
    7d73:	8b 43 14             	mov    0x14(%ebx),%eax
    7d76:	89 44 24 04          	mov    %eax,0x4(%esp)
    7d7a:	8b 43 08             	mov    0x8(%ebx),%eax
    7d7d:	25 ff ff ff 00       	and    $0xffffff,%eax
    7d82:	89 04 24             	mov    %eax,(%esp)
    7d85:	e8 50 ff ff ff       	call   7cda <readseg>
    goto bad;
  ...
  eph = ph + elf->phnum;
  for(; ph < eph; ph++)
    7d8a:	83 c3 20             	add    $0x20,%ebx
    7d8d:	3b 5d f4             	cmp    -0xc(%ebp),%ebx
    7d90:	72 da                	jb     7d6c <bootmain+0x4d>
    readseg(ph->va & 0xFFFFFF, ph->memsz, ph->offset);

  // Call the entry point from the ELF header.
  // Does not return!
  entry = (void(*)(void))(elf->entry & 0xFFFFFF);
    7d92:	a1 18 00 01 00       	mov    0x10018,%eax
    7d97:	25 ff ff ff 00       	and    $0xffffff,%eax
  entry();
    7d9c:	ff d0                	call   *%eax
}
...
{
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
    7d9e:	ba 00 8a ff ff       	mov    $0xffff8a00,%edx
    7da3:	b8 00 8a ff ff       	mov    $0xffff8a00,%eax
    7da8:	66 ef                	out    %ax,(%dx)
    7daa:	b8 00 8e ff ff       	mov    $0xffff8e00,%eax
    7daf:	66 ef                	out    %ax,(%dx)
    7db1:	eb fe                	jmp    7db1 <bootmain+0x92>
